...
{
    // For simplicity, I've just set the destination branch's latest snapshot to the source's
    // snapshot A real implementation might involve three-way merges, conflict resolution, etc.
    if (branches.find(source_branch) == branches.end() ||
        branches.find(dest_branch) == branches.end()) {
...
std::string BranchManager::get_current_branch_name() { return current_branch; }
